...
class ParameterSetPartForm(forms.ModelForm):
    ...
    instruction_set = forms.ModelChoiceField(label='Instruction Set',
                                            ...
                                            widget=forms.Select(attrs={"v-model":"current_parameter_set_part.instruction_set.id"}))


    class Meta:
        model=ParameterSetPart
        fields =['mode', 'minimum_for_majority', 'pay_choice_majority', 'pay_choice_minority', 'pay_label_majority', 'pay_label_minority', 'instruction_set']
